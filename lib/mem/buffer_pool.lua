...
Verditer.lib.mem.BufferPool = BufferPool
...
function BufferPool:acquire()
  ...
  if Verditer.Validation then
    Verditer.Validation.pool_acquired(self._label, rec)
  end
  return rec
end
...
function BufferPool:release(rec)
  ...
  if Verditer.Validation then
    Verditer.Validation.pool_released(self._label, rec)
  end
