...
local function emit(level, source, ...)
  ...
  local debug_on = Verditer.Constants.DEBUG
  if not debug_on and level ~= "err" then return end
  local n = select("#", ...)
  local args = { ... }
  local line = format_line(level, source, args, n)
  if debug_on and Verditer.CopyBox then
    Verditer.CopyBox.append(line)
...
function M.write(level, key, data)
  ...
  if (level == "warn" or level == "error") and Verditer.CopyBox then
    local body = "[" .. level .. ":" .. key .. "]"
    if data ~= nil then
      body = body .. " " .. format_data(data)
    end
    Verditer.CopyBox.append(body)
  end
end

function M.flush()
  if not Verditer.SavedVars then return 0 end
  Verditer.SavedVars.debug = Verditer.SavedVars.debug or {}
  local out = {}
  ...
  Verditer.SavedVars.debug.log = out
  return #out
end
...
function M.show_recent(n)
  ...
  if Verditer.CopyBox and Verditer.CopyBox.show then
    Verditer.CopyBox.show("Verditer /log show", table.concat(lines, "\n"))
  else
