...
local function record_failure(check, details)
  failures[#failures+1] = {
    t       = Verditer.zenimax.api.GetGameTimeMilliseconds(),
    check   = check,
    details = details,
  }
  if Verditer.Log and Verditer.Log.write then
    Verditer.Log.write("error", "validation." .. check, details)
  end
end
...
function M.dump_to_chat()
  ...
  if Verditer.CopyBox and Verditer.CopyBox.show then
    Verditer.CopyBox.show("Verditer /validate", table.concat(lines, "\n"))
  end
end
