...
function M.exit(name)
  if stack_top == 0 then
    if Verditer.Log and Verditer.Log.write then
      Verditer.Log.write("error", "profiler.unbalanced", { exit = name, stack_top = 0 })
    end
    return
  end
  local frame = enter_stack[stack_top]
  if frame.name ~= name then
    if Verditer.Log and Verditer.Log.write then
      Verditer.Log.write("error", "profiler.unbalanced",
...
function M.dump_to_chat()
  ...
  if Verditer.CopyBox and Verditer.CopyBox.show then
    Verditer.CopyBox.show("Verditer /prof", table.concat(lines, "\n"))
  end
end
